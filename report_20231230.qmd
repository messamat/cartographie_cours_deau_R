...
#Check if at least 1% of nce to determine whether they were included
deps_wnce_summary <- drainage_density_summary$nets_stats_melt_dep[
  !duplicated(INSEE_DEP),
  list(nce_o1perc = per_nce > 0.01), by=INSEE_DEP]
...
summary_table_raw <- Reduce(
  ...
  list(meta_summary, deps_wnce_summary,
       per_missing_area_bydep, vulnwat_summary,
       ddr_summary, gclass_summary)
) %>%
  .[, nce_o1perc := (
    per_nce>0.01|(per_nce>0 & per_nce_interp<0.01 & stats_source == 'bdtopo') |
      (per_nce>0 & lengthratio_ddt_ceind_to_other>=1))] %>%
  .[, c(
    "INSEE_DEP","dep_name", 'gclass', 'data_orig', 'n_bvs',"nlines_reported", "length_ddtnets_total",'nce_included', 'nce_included_cat', 'nce_o1perc', 'per_missing_area', "lengthratio_ddt_ceind_to_other", 'per_ce', 'per_ind', 'per_nce','stats_source', 'length_order','per_determined', 'per_nce_interp', 'per_ires', "per_hdw", "per_hdworires", "ires_nce_representativeness", "per_nce_hdworires","representativeness_hdworires_in_nce"
  ),
  with=F] 
...
mod_eqs <- lapply(mods_envdd, function(cl) {
...
'Non-perennial length (km)', '% non-watercourse',

'% non-perennial',

'% non-watercourse and non-perennial',

'% length with flow permanence status',

'Strahler order representativeness',

'Non-perennial representativeness'
...
out_tab[, strahler := factor(str_to_title(strahler),
                             levels=c('1', '2', '3', '4', 'Undetermined', 'Total'))]
