...
### Histograms to accompany national maps of drainage density

```{r department_summary_histogram}
#| label: tbl-summary-dep 
#| tbl-cap: "" 

# setnames(summary_table_raw,
#          c('% watercourses (mapped)', '% uncategorized (mapped)',
#            '% n-w (mapped)', 'Drainage density ratio (mapped)'),
#          c('ce', 'ind', 'nce', 'ddratio'))

#Watercourses ----------------------------------------------------
summary_table_raw$ce_interval <- cut(
  summary_table_raw$ce,
  breaks = c(0, 0.2, 0.4, 0.6, 0.8, 0.95, 1),
  include.lowest = FALSE,
  labels = c("0-0.20", "0.20-0.40", "0.40-0.60", 
             "0.60-0.80", "0.80-0.95", "0.95-1.00"))

ggplot(summary_table_raw[!(Name %in% c(75, 92, 93, 94)),], 
       aes(x=ce, fill=ce_interval)) +
...
#Unclassified ------------------------------------------------
summary_table_raw$ind_interval <- cut(
  summary_table_raw$ind,
  breaks = c(0, 0.001, seq(0.05,1,0.05)),
  include.lowest = T
)

colscale_unclassified <- c('#fff7f3ff', rep('#fcd0ccff', 4), rep('#f994b1ff', 4),
  rep('#e23e99ff', 4),rep('#99007bff', 4),rep('#49006aff', 4))
names(colscale_unclassified) <- levels(summary_table_raw$ind_interval)

colscale_unclassified_dt <- as.data.table(colscale_unclassified, 
                                          keep.rownames = T) %>%
  .[, order:= .I]

summary_table_raw_indcount <- summary_table_raw[, .N, by=ind_interval] %>% 
  merge(colscale_unclassified_dt, ., by.x='rn', by.y='ind_interval', all.x=T) %>%
  .[is.na(N), N := 0] %>%
  .[, ind_interval := factor(rn, levels=colscale_unclassified_dt$rn)]
  
ggplot(summary_table_raw_indcount, 
       aes(x=ind_interval, y=N, fill=ind_interval)) +
  geom_bar(stat='identity', witdh=1) +
  scale_x_discrete(labels=c('0', rep('', 3), '0.20', rep('', 3), '0.40',
                            rep('', 3), '0.60', rep('', 3), 0.80, rep('', 3), 1.00)) +
